...
RUN apk add --update --no-cache --virtual deps gcc zlib-dev jpeg-dev build-base linux-headers freetype-dev \
    && apk add --no-cache python3 python3-dev py3-pip py3-pyserial py3-usb py3-pymysql sqlite wget rsync openssh alpine-conf \
    && ln -sf python3 /usr/bin/python \
    && pip3 install --no-cache --upgrade Cheetah3 Pillow image pyephem setuptools requests dnspython paho-mqtt configobj \
    && apk del py3-pip deps \
    && cd /tmp/weewx \
